...
addProgramIdToPlanSections = (dataDir, globalEncryptionKey, cb) ->
	...
	, (err) ->
		if err
			console.info "Problem with planSections programId"
			cb err
			return
...
module.exports = {
	run: (dataDir, userName, password, lastMigrationStep, cb) ->
		...
		migrationSeries = [
			...
			(cb) ->
				console.groupEnd()
				console.groupCollapsed "1. Add 'programId': ' ' field to plan sections"
				addProgramIdToPlanSections dataDir, globalEncryptionKey, cb

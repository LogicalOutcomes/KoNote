...
addCustomIdToMetrics = (dataDir, globalEncryptionKey, cb) ->
				...
				(cb) =>
					Fs.readFile metricRevPath, (err, result) ->
						if err
							cb err
							return
						metricRevObj = Imm.fromJS(JSON.parse globalEncryptionKey.decrypt result)
						#console.log "original metricRevObj: ", JSON.parse globalEncryptionKey.decrypt result
						cb()

				(cb) =>
					# add customId to each metric revision
					metricRevObj = metricRevObj
					.set 'customId', ''
					.toJS()

					#console.log "migrated metricRevObj: ", metricRevObj
					encryptedObject = globalEncryptionKey.encrypt(JSON.stringify metricRevObj)
					Fs.writeFile metricRevPath, encryptedObject, cb
			], cb
		, cb
	, (err) ->
			if err
				console.warn "Problem with metric migration"
				cb err
				return
			finalizeMigrationStep(dataDir, cb)

addCustomIdIndex = (dataDir, globalEncryptionKey, cb) ->
	...
	forEachFileIn metricsDir, (metric, cb) ->
		...
		indexes = decryptFileName metric, 3, globalEncryptionKey
		indexes.splice -1, 0, ''
	...
	, (err) ->
		if err
			console.info "problem with metrics customId index"
			cb err
			return
...
module.exports = {
	run: (dataDir, userName, password, lastMigrationStep, cb) ->
		...
		migrationSeries = [
			...
			(cb) ->
				console.groupEnd()
				console.groupCollapsed "1. Add 'customId': ' ' field to metrics"
				addCustomIdToMetrics dataDir, globalEncryptionKey, cb

			(cb) ->
				console.groupEnd()
				console.groupCollapsed "2. Add 'customId' index to metrics"
				addCustomIdIndex dataDir, globalEncryptionKey, cb

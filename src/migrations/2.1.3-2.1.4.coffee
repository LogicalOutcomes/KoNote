	...
	Async.series [
		...
		(cb) ->
			...
			Async.eachSeries fileNames, (fileName, cb) ->
				...
				if fileName in ['.DS_Store', 'Thumbs.db', 'progEvents', 'planTargets', 'progNotes', 'alerts', 'attachments']
...
addProgramIdToTemplateSections = (dataDir, globalEncryptionKey, cb) ->
	forEachFileIn Path.join(dataDir, 'planTemplates'), (planTemplate, cb) ->
		templateDirPath = Path.join(dataDir, 'planTemplates', planTemplate)

		forEachFileIn templateDirPath, (templateRev, cb) ->
			templateRevPath = Path.join(templateDirPath, templateRev)
			templateRevObject = null

			Async.series [
				(cb) =>
					# Read and decrypt template object
					Fs.readFile templateRevPath, (err, result) ->
						if err
							cb err
							return

						# console.log JSON.parse globalEncryptionKey.decrypt result

						templateRevObject = Imm.fromJS(JSON.parse globalEncryptionKey.decrypt result)

						cb()
				(cb) =>
					# Add empty programId to every section
					planSections = templateRevObject.getIn(['sections']).map (section) ->
						return section.set 'programId', ''

					templateRevObject = templateRevObject
					.setIn(['sections'], planSections)
					.toJS()

					console.log "templateRevObject", templateRevObject

					encryptedObject = globalEncryptionKey.encrypt(JSON.stringify templateRevObject)

					Fs.writeFile templateRevPath, encryptedObject, cb
			], cb

		, cb
	, (err) ->
		if err
			console.info "Problem with template section programId"
			cb err
			return
...
module.exports = {
	run: (dataDir, userName, password, lastMigrationStep, cb) ->
		...
		migrationSeries = [
			...
			(cb) ->
				console.groupEnd()
				console.groupCollapsed "1. Add 'programId': ' ' field to template sections"
				addProgramIdToTemplateSections dataDir, globalEncryptionKey, cb
